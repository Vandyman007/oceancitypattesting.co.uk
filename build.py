...
def write_htaccess(root):
    """Apache/LiteSpeed rules for 20i (shared) hosting.

    The site links to clean, extensionless URLs (/about, /services/pat-testing)
    but the files on disk are about.html etc. Cloudflare Pages resolved that
    automatically; Apache does not, so we rewrite here. Note the deliberate
    file-vs-directory collision: /areas-covered and /services each have BOTH a
    .html page and a same-named directory of children — the .html wins for the
    bare path because we test for the file before letting mod_dir act.
    """
    htaccess = """# ---- Generated by build.py — do not edit by hand ----
Options -MultiViews -Indexes
DirectorySlash Off
RewriteEngine On

# Force HTTPS. 20i terminates TLS at its edge, so the origin always sees
# %{HTTPS}=off — test the forwarded proto instead, or this loops forever.
RewriteCond %{HTTP:X-Forwarded-Proto} =http
RewriteRule ^ https://%{HTTP_HOST}%{REQUEST_URI} [L,R=301]

# Canonical host: send apex -> www to match CNAME / og:url
RewriteCond %{HTTP_HOST} ^oceancitypattesting\\.co\\.uk$ [NC]
RewriteRule ^ https://www.oceancitypattesting.co.uk%{REQUEST_URI} [L,R=301]

# 301 any explicit .html request to its clean URL
RewriteCond %{THE_REQUEST} \\s/+([^?\\s]+)\\.html[\\s?] [NC]
RewriteRule ^ /%1 [R=301,L]

# 301 a trailing slash (except the site root and real directories) to the bare URL
RewriteCond %{REQUEST_FILENAME} !-d
RewriteRule ^(.+?)/$ /$1 [R=301,L]

# Internally serve <path>.html when the clean URL has no matching file.
# We test the .html twin BEFORE excluding directories so the .html page wins
# the collision on /areas-covered and /services.
...
# Security headers (ported from the Cloudflare _headers file)
...
# The whole repo deploys to the web root — keep source/tooling private
<FilesMatch "\\.(py|md)$">
  Require all denied
</FilesMatch>
RedirectMatch 404 (?i)/(\\.git|tools)(/|$)
...
"""
    with open(os.path.join(root, ".htaccess"), "w", encoding="utf-8") as f:
        f.write(htaccess)
    print("  wrote .htaccess (20i/Apache clean-URL + headers)")
